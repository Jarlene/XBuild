...
namespace xbuild {
    ...
    const char *assert_string_valid(const char *name, const char *data, size_t size) {
        assert(data <= name && name + strlen(name) + 1 <= data + size);
        return name;
    }
...
// of the target type.
    ...
    T safe_cast(U x) {
        assert(std::numeric_limits<T>::min() <= x && x <= std::numeric_limits<T>::max());
...
// above to validate the assignment.
    ...
    std::unique_ptr<Object> parse_object_internal(const char *data, size_t size) {
        ...
        assert(header.e_type == Object::ET_REL || header.e_type == Object::ET_DYN);
        ...
        auto get_section_header = [&](int idx) -> const Shdr<T> * {
            ...
            assert(data <= at && (at + sizeof(Shdr<T>)) <= data + size);
        ...
        for (int i = 0; i < header.e_shnum; i++) {
            ...
            if (sh->sh_type == Section::SHT_STRTAB) {
                ...
                assert(data <= strings && strings + sh->sh_size <= data + size);
            }
        }
        assert(strings);
        ...
        for (uint16_t i = 0; i < header.e_shnum; i++) {
            ...
            if (sh->sh_type != Section::SHT_SYMTAB && sh->sh_type != Section::SHT_STRTAB &&
                ...
                if (sh->sh_type == Section::SHT_NOBITS) {
                    ...
                    assert(data <= sh_data && sh_data + sh->sh_size <= data + size);
        ...
        for (uint16_t i = 0; i < header.e_shnum; i++) {
            ...
            if (sh->sh_type == Section::SHT_SYMTAB) {
                assert(sh->sh_entsize == sizeof(Sym<T>));
                ...
                for (uint64_t j = 1; j < sh->sh_size / sizeof(Sym<T>); ++j) {
                    ...
                    assert(data <= sym_ptr && sym_ptr + sizeof(Sym<T>) <= data + size);
    ...
    Object::section_iterator Object::merge_sections(const std::vector<section_iterator> &to_merge) {
        ...
        for (auto i = to_merge.begin() + 1; i != to_merge.end(); ++i) {
            ...
            assert(s->get_type() == merged->get_type());
    ...
    Object::section_iterator Object::merge_text_sections() {
        ...
        for (auto i = sections_begin(); i != sections_end(); i++) {
            if (i->get_type() == Section::SHT_PROGBITS && startWith(i->get_name(), ".text")) {
    ...
    template<typename T>
    std::vector<char>
    write_shared_object_internal(Object &obj, Linker *linker, const std::vector<std::string> &dependencies,
...
//            debug(2) << "Writing section " << s.get_name() << " at offset " << offset << "\n";
        ...
        if (iter_ctors != obj.sections_end()) {
            ...
            assert(ctors->get_size() == ctors->contents_size());
...
//                debug(2) << "Defining PLT entry for " << sym->get_name() << "\n";
        ...
        std::sort(sorted_symbols.begin(), sorted_symbols.end(),
                  [&](const std::pair<const Symbol *, const Symbol *> &lhs,
                      const std::pair<const Symbol *, const Symbol *> &rhs) {
...
//                    debug(2) << "Symbol " << sym->get_name() << " is defined at " << sym_offset << "\n";
                ...
                Relocation new_reloc = linker->relocate(fixup_offset, fixup_addr, r.get_type(), sym, sym_offset,
                                                        r.get_addend(), got);
...
//                    debug(2) << "Linker returned new relocation type " << new_reloc.get_type() << "\n";
        ...
        assert(got.contents_size() <= max_got_size);
